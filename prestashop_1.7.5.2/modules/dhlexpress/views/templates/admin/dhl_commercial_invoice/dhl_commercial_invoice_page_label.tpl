        ...
        <div class="col-lg-12">
          <div class="panel form-horizontal clearfix">
            ...
            <div class="form-group">
              ...
              <div class="col-lg-3">
                <input type="text" name="dhl_total_package" id="dhl-total-package" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');" 
                    ...
                    <tr>
                      ...
                      <td>
                        <select class="fixed-width-lg" name="dhl_supp_country" id="dhl-supp-country">
                          {foreach $countries as $country}
                            <option value="{$country.id_country|intval}" {if $country.id_country == $default_country}selected="selected"{/if}>
  ...
  <script type="text/javascript">
    ...
    $('#submit-dhl-invoice-download').click(function (e) {
      ...
      $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '{/literal}{$link->getAdminLink('AdminDhlCommercialInvoice')|escape:'urldecode'}{literal}&ajax=1&action=generateInvoiceInPageLabel',
    ...
    $('#submit-dhl-add-sup-product').click(function (e) {
      $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '{/literal}{$link->getAdminLink('AdminDhlCommercialInvoice')|escape:'urldecode'}{literal}&ajax=1&action=addSupProduct',
        data: $('#create-label').serialize(),
        success: function (data) {
          if (data.errors !== true) {
            addI++;
