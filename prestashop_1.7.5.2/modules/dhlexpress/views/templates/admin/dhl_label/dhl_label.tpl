    ...
    <div class="row">
      <div class="col-lg-12">
        <div class="panel form-horizontal clearfix">
            ...
            <div id="dhl-package-{$package_type.id_dhl_package|intval}" class="form-group dhl-packages"
              ...
              <div class="col-lg-4 col-lg-offset-3">
                ...
                <div class="input-group fixed-width-xs">
                  <input onchange="this.value = parseFloat(this.value.replace(/,/g, '.')) || 0" name="dhl_package_weight_{$package_type.id_dhl_package|intval}" type="text"
                         value="{$package_type.weight_value|floatval}" class="fixed-width-xs dhl-value-weight">
                ...
                <div class="input-group fixed-width-xs">
                  <input onchange="this.value = parseInt(this.value) || 0" name="dhl_package_length_{$package_type.id_dhl_package|intval}" type="text"
                         value="{$package_type.length_value|floatval}" class="fixed-width-xs dhl-value-length">
                ...
                <div class="input-group fixed-width-xs">
                  <input onchange="this.value = parseInt(this.value) || 0" name="dhl_package_width_{$package_type.id_dhl_package|intval}" type="text"
                         value="{$package_type.width_value|floatval}" class="fixed-width-xs dhl-value-width">
                  <span class="input-group-addon dhl-suffix-dimension">{$dimension_unit|escape:'html':'utf-8'}</span>
                </div>
                <label class="control-label col-lg-3">{l s='Depth' mod='dhlexpress'}</label>

                <div class="input-group fixed-width-xs">
                  <input onchange="this.value = parseInt(this.value) || 0" name="dhl_package_depth_{$package_type.id_dhl_package|intval}" type="text"
                         value="{$package_type.depth_value|floatval}" class="fixed-width-xs dhl-value-depth">
                  <span class="input-group-addon dhl-suffix-dimension">{$dimension_unit|escape:'html':'utf-8'}</span>
                </div>
          ...
          <div class="dhl-process-button">
            ...
            <div style="clear: both; display: none;" class="panel_plt panel col-lg-12" id="panel_plt">
                <div style="border-bottom: solid 1px #a0d0eb;margin-bottom: 15px;"><span style="line-height: 3;" class="title_box">{l s="Use P.L.T. Option to save time and paper ! No need to attach 'physically ' your DHL commercial invoice, send it by electronic transmission : PaperLess Transfer" mod='dhlexpress'}</span></div>
                <form>
                    <input type="radio" name="dhl_plt_service" id="use_plt_option" value="create_invoice" checked="true" style="margin-right: 10px;" onchange="usePltOption();" /> {l s="Use P.L.T. Option by editing commercial invoice here :" mod='dhlexpress'}
                        <a onclick="displayFormInvoice();" style="text-transform: none;margin-left: 30px;{*display: none;*}" title="Create DHL invoice" class="btn_display_form_invoice edit btn btn-default btn-primary">
                    ...
                    <input type="radio" name="dhl_plt_service" id="use_plt_option2" value="upload_invoice" style="margin-right: 10px;" onchange="uploadOwnInvoice();" /> {l s="Use P.L.T. Option and - upload your own PDF invoice" mod='dhlexpress'}
                        ...
                        <div class="form-group" style="margin-bottom: 0;">
                            <div>
                                <div id="view_file_pdf" class="btn-group" style="display: none; text-transform: none;">
                                    <a target="_blank" id="edit_file_pdf"  style="text-transform: none;"
                                        href=""
                                        title="View invoice" class="edit btn btn-default btn-primary" style="text-transform: none; border-right: 2px solid #2eacce;">
                                ...
                                <form method="post" id="form-upload_invoice" onsubmit="return false" enctype="multipart/form-data">
                                  <button id="dhl_upload_invoice" type="submit" class="btn btn-default pull-right btn-primary" name="submit_dhl_upload_invoice" style="margin-right: 22%; text-transform: none;"> {l s="save" mod='dhlexpress'} </button>
                                  {$input_upload}
                                  <p id="error_pdf_form" class="help-block" style="font-size: 10px;">{l s="Only .pdf files are allowed" mod='dhlexpress'}</p>
                                  <p id="error_champ_vide" class="help-block" style="font-size: 10px; color: red; display: none;">{l s="Insert the invoice please " mod='dhlexpress'}</p>
                                </form>
                            </div>
                    ...
                    <br/><br/><br/><br/>
                    <input type="radio" name="dhl_plt_service" id="use_plt_option3" value="not_use_plt" style="margin-right: 10px;" onchange="notUsePlt();" /> {l s="Do not use P.L.T. and edit label (I asume that I have to attach my commercial invoice to my shipping)" mod='dhlexpress'}
                </form>
            ...
            {if $action == 'create'}
                <div class="alert alert-info msg_plt_not_eligible" style="display: none; clear: both;">
                    <ul>
                        <li><span>{l s="Shipping country or order amount is not eligible to Paperless Transfer (PLT). You should attached your commercial invoice paper document to your shipping." mod='dhlexpress'}</span></li>
                     {*   {else if ($action == 'createreturn')}
                            <li><span>{l s="La plt n/'est pas autorisee en cas de creation return label." mod='dhlexpress'}</span></li>*}
                    </ul>
                </div>
            {/if}
        ...
        <div id="dhl-generate-label-and-invoice-block" class="dhl-process-button" style="display: none;">
            ...
            <button id="submit-dhl-label-invoice-create" type="submit" class="btn btn-primary" name="submitDhlLabelCreate" >
...
<script type="text/javascript">
  ...
  $('#submit-dhl-label-prices').click(function (e) {
    ...
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: '{/literal}{$link->getAdminLink('AdminDhlLabel')|escape:'urldecode'}{literal}&ajax=1&action=retrieveDhlService',
  ...
  $('#submit-dhl-label-create').click(function (e) {
    ...
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: '{/literal}{$link->getAdminLink('AdminDhlLabel')|escape:'urldecode'}{literal}&ajax=1&action=generateFormLabel',
      data: $("#create-label").serialize(),
      success: function (data) {
  ...
  $('#submit-dhl-label-invoice-create').click(function (e) {
    var dhlLabelResult = $('.dhl-label-result');
    //console.log($("#create-label").serialize());
    dhlLabelResult.html('');
    $('#dhl-loading-save').show();
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: '{/literal}{$link->getAdminLink('AdminDhlLabel')|escape:'urldecode'}{literal}&ajax=1&action=generateFormLabelAndSaveInvoice',
      data: $("#create-label").serialize(),
      success: function (data) {
  ...
  $('.dhl-add-package').click(function (e) {
    ...
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: '{/literal}{$link->getAdminLink('AdminDhlLabel')|escape:'urldecode'}{literal}&ajax=1&action=addDhlPackage',
      data: 'id_dhl_package=' + idPackage + '&width=' + width + '&length=' + length + '&depth=' + depth + '&weight=' + weight,
      success: function (data) {
        if (data.errors !== true) {
 ...
 $('#dhl_upload_invoice').click(function (e) {
        ...
        var ajaxUrl = '{$link->getAdminLink('AdminDhlLabel')|escape:'urldecode'}&ajax=1&action=uploadPdfInvoice';
        $.ajax({
                url : ajaxUrl,
                type : "POST",
                data : formData,
...
//            there no error
  ...
  $('#dhl-edit-invoice').click(function (e) {
    $.ajax({
      type: 'POST',
      dataType: 'json',
      url: '{$link->getAdminLink('AdminDhlLabel')|escape:'urldecode'}&ajax=1&action=deleteInvoice',
      data: $('#create-label').serialize(),
      success: function (data) {
