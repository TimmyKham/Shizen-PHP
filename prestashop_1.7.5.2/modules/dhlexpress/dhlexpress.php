...
class Dhlexpress extends CarrierModule
{
    ...
    public function __construct()
    {
        ...
        $this->tabs = array(
            ...
            array(
                ...
                'menu_names' => array('en' => 'Commerciale invoice for exportation', 'fr' => 'DHL Facture commerciale pour l\'export'),
                'active' => 0,
            ),
        );
    }
    ...
    public function installTab($className, $menuName, $active)
    {
        ...
        foreach (Language::getLanguages(true) as $lang) {
            $tab->name[$lang['id_lang']] = isset($menuName[$lang['iso_code']]) ? pSQL($menuName[$lang['iso_code']]) : pSQL($menuName['en']);
        }
    ...
    public function createServices($idCarrier, $dhlServices, $languages)
    {
        ...
        foreach ($services as $service) {
            $idDhlService = DhlService::getServiceByIdCarrierDestination(
                $idCarrier, $service['destination_type'], $service['doc'], true
            );
	...
	$return['Extracharges'] = array(
        ...
        if (!$key) {
            return array_merge(
                $return['BillingAccount'], $return['SenderAddress'], $return['ShipmentDetails'], $return['InvoiceDetails'], $return['Extracharges']
            );
        }
    ...
    public function getBackOfficeSettingsFormValues()
    {
        ...
        return array_merge(
            $defaultPackageArray, $defaultAddressArray, $extrachargeArray, Configuration::getMultiple($this->getBackOfficeSettingsFormFields()), $signature_value
        );
    }
    ...
    public function getPackageShippingCost($cart, $shippingCost, $products)
    {
        ...
        $globalProductCode = DhlService::getProductCodeByIdCarrierDestination(
            $idCarrierReference, $destinationType, $doc
        );
                                ...
                                if (isset($dhlCheckpoints[$dhlCode])) {
                                    ...
                                    if (!$update) {
                                        ...
                                        if ($dhlLabel->getLastTrackingStatusKnown()) {
                                            $logger->info(
                                                'Update shipment_tracking row.', array('id_dhl_tracking' => $idDhlTracking)
                                            );
                                            Db::getInstance()->update(
                                                'dhl_shipment_tracking', array(
                                            ...
                                            $logger->info(
                                                'Create shipment_tracking row.', array('id_dhl_tracking' => $idDhlTracking)
                                            );
                                            ...
                                            DhlTools::sendInTransitMail(
                                                $order, $customer, $carrier, $awbNumber
                                            );
                                        }
                                        $update = 1;
                                    }
